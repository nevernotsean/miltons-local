...
class WC_Constant_Contact_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		$options = array(
			...
			'description' => __( 'Allow your customers to subscribe to your Constant Contact email list.', 'woocommerce-constant-contact' ),
		);

		// instantiate the widget
		parent::__construct( 'wc_constant_contact', __( 'WooCommerce Constant Contact', 'woocommerce-constant-contact' ), $options );
	...
	public function widget( $args, $instance ) {
		...
		?>
			<form method="post" id="wc_constant_contact_subscribe_widget_form" action="#wc_constant_contact_subscribe_widget_form">
				<div>
					<label class="screen-reader-text hidden" for="s"><?php esc_html_e( 'Email Address', 'woocommerce-constant-contact' ); ?>:</label>
					<input type="hidden" id="wc_constant_contact_subscribe_list_id" value="<?php echo $list_id; ?>" />
					<input type="text" name="wc_constant_contact_subscribe_email" id="wc_constant_contact_subscribe_email" placeholder="<?php esc_attr_e( 'Your email address', 'woocommerce-constant-contact' ); ?>" />
					<input type="submit" id="wc_constant_contact_subscribe" value="<?php esc_attr_e( 'Subscribe', 'woocommerce-constant-contact' ); ?>" />
				</div>
			</form>
		<?php
	...
	public function form( $instance ) {
		...
		if ( ! wc_constant_contact()->get_api() ) {
			?><p><?php esc_html_e( 'You must enter your API info in  WooCommerce > Settings > Constant Contact before using this widget.', 'woocommerce-constant-contact' ); ?></p><?php
			return;
		}
		...
		?>
			<p><label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title', 'woocommerce-constant-contact' ) ?>:</label>
				<input type="text" class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" value="<?php echo esc_attr( ( isset( $instance['title'] ) ) ? $instance['title'] : __( 'Email List', 'woocommerce-constant-contact' ) ); ?>" /></p>
			<p><label for="<?php echo esc_attr( $this->get_field_id( 'list' ) ); ?>"><?php esc_html_e( 'Email List', 'woocommerce-constant-contact' ); ?></label>
				<?php
				...
				echo '<small>' . esc_html__( 'Select an email list to subscribe customers to or leave blank to use the list defined under WooCommerce > Settings > Constant Contact.', 'woocommerce-constant-contact' ) . '</small>';
				?>
			</p>
		<?php
	}
	...
	public function ajax_process_widget_subscribe() {
		...
		if ( ! is_email( $email ) ) {
			...
			$error = '<div class="woocommerce"><div class="woocommerce-error">' . __( 'Please enter a valid email address.', 'woocommerce-constant-contact' ) . '</div></div>';
			...
			try {
				...
				$success = '<div class="woocommerce"><div class="woocommerce-message">' . __( 'Thanks for subscribing.', 'woocommerce-constant-contact' ) . '</div></div>';

				wp_send_json_success( $success );

			} catch ( SV_WC_API_Exception $e ) {

				$error = '<div class="woocommerce"><div class="woocommerce-error">' . __( 'Oops, something went wrong. Please try again later.', 'woocommerce-constant-contact' ) . '</div>';

				wp_send_json_error( $error );

				wc_constant_contact()->log( sprintf( __( 'Widget Signup: %s', 'woocommerce-constant-contact' ), $e->getMessage() ) );
			}
		}
	}
