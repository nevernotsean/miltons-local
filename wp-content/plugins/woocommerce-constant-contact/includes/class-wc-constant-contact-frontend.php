...
class WC_Constant_Contact_Frontend {
	...
	public function process_subscribe_checkbox( $order_id ) {
		...
		try {
			...
			$order->add_order_note( sprintf( __( 'Failed to subscribe customer to email list: %s', 'woocommerce-constant-contact' ), $e->getMessage() ) );
		}
	}
	...
	public function render_order_received_subscribe_section( $order_id ) {
		...
		$message .= '<a href="#" id="wc_constant_contact_subscribe" class="button">' . apply_filters( 'wc_constant_contact_order_received_button_text', __( 'Subscribe Now', 'woocommerce-constant-contact' ) ) . '</a>';

		// wrap with info div
		$message = '<div class="woocommerce-info wc_constant_contact_order_received_subscribe_section">' . $message . '</div>';

		echo apply_filters( 'wc_constant_contact_order_received_subscribe_message', $message, $order_id );
	...
	public function ajax_process_order_received_subscribe() {
		...
		if ( ! $order_id ) {
			...
			wc_add_notice( apply_filters( 'wc_constant_contact_order_received_subscribe_failure_message', __( 'Oops, something went wrong. Please contact us to subscribe.', 'woocommerce-constant-contact' ) ), 'error' );
			wc_print_notices();

			wc_constant_contact()->log( __( 'Order Received Subscribe: No Order ID', 'woocommerce-constant-contact' ) );
		...
		try {
			...
			wc_add_notice( apply_filters( 'wc_constant_contact_order_received_subscribe_success_message', __( 'Thanks for subscribing!', 'woocommerce-constant-contact' ) ) );
			...
			wc_constant_contact()->log( sprintf( __( 'Order Received Subscribe: %s', 'woocommerce-constant-contact' ), $e->getMessage() ) );
		}
