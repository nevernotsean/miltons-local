...
class WC_Constant_Contact_Settings extends WC_Settings_Page {
	...
	public function get_settings() {
		...
		return apply_filters( 'wc_constant_contact_settings', array(
			...
			array(
				...
				'desc'    => sprintf( __( 'Save API requests/responses and Detailed Error Messages to the debug log: %s', 'woocommerce-constant-contact' ), '<strong class="nobr">' . wc_get_log_file_path( wc_constant_contact()->get_id() ) . '</strong>' ),
				'default' => 'off',
				'type'    => 'select',
				'options' => array(
			...
			?>
				<tr valign="top">
					<th scope="row" class="titledesc">
						...
						<?php SV_WC_Plugin_Compatibility::wc_help_tip( $field['desc_tip'] ); ?>
					</th>
	 ...
	 *
	 * WC 2.4+ wc_clean()'s the option value before saving it 
