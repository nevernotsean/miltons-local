...
class WC_Constant_Contact_Points_and_Rewards {
	...
	public function add_settings( $settings ) {
		...
		$settings = array_merge(
			...
			array(
				array(
					'title'    => __( 'Points earned for a Constant Contact sign up', 'woocommerce-constant-contact' ),
					'desc_tip' => __( 'Enter the amount of points earned when a customer signs up to an email list via Constant Contact.', 'woocommerce-constant-contact' ),
	...
	public function render_points_description( $description, $event_type, $event ) {
		...
		return sprintf( __( '%s earned for email sign-up', 'woocommerce-constant-contact' ), $points_label );
	}
