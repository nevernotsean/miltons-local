<?php
/**
 * Plugin Name: WooCommerce Constant Contact
 * Plugin URI: http://www.woothemes.com/products/woocommerce-constant-contact/
 * Description: Easily signup customers to your Constant Contact email lists during checkout and anywhere else on your WooCommerce store with the included widget!
 * Author: WooThemes / SkyVerge
 * Author URI: http://www.woothemes.com
 * Version: 1.7.3
 * Text Domain: woocommerce-constant-contact
...
// Plugin updates
woothemes_queue_update( plugin_basename( __FILE__ ), '0f254fc9658047f5ef543d883c9c7786', '216809' );
...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.5.1', __( 'WooCommerce Constant Contact', 'woocommerce-constant-contact' ), __FILE__, 'init_woocommerce_constant_contact', array(
...
class WC_Constant_Contact extends SV_WC_Plugin {
	...
	/**
	 * Backwards compat for changing the visibility of class instances
	 *
	...
	public function __get( $name ) {
		...
		switch ( $name ) {
			...
			case 'frontend':
				...
				_deprecated_function( 'wc_constant_contact()->admin', '1.7.0', 'wc_constant_contact()->get_frontend_instance()' );
				return $this->get_frontend_instance();

			case 'points_and_rewards':
				/* @deprecated since 1.7.0 */
				_deprecated_function( 'wc_constant_contact()->points_and_rewards', '1.7.0', 'wc_constant_contact()->get_points_and_rewards_instance()' );
				return $this->get_points_and_rewards_instance();

			case 'settings':
				/* @deprecated since 1.7.0 */
				_deprecated_function( 'wc_constant_contact()->settings', '1.7.0', 'wc_constant_contact()->get_settings_instance()' );
				return $this->get_settings_instance();
	...
	public function load_translation() {
		...
		load_plugin_textdomain( 'woocommerce-constant-contact', false, dirname( plugin_basename( $this->get_file() ) ) . '/i18n/languages' );
	}
	...
	public function render_dashboard() {
		...
		if ( empty( $stats ) ) {
			...
			echo '<div class="error inline"><p>' . __( 'Unable to load stats from Constant Contact', 'woocommerce-constant-contact' ) . '</p></div>';
	...
	public function get_documentation_url() {
	    ...
	    return 'http://docs.woothemes.com/document/woocommerce-constant-contact/';
	}
