...
if (! class_exists('SLPlus_AdminUI_UserExperience')) {
	...
	class SLPlus_AdminUI_UserExperience extends SLPlus_BaseClass_Object {
			...
			$this->settings->add_ItemToGroup( array(
				...
				'description' =>
					...
					sprintf(
						...
						$this->slplus->get_product_url('slp-premier')
					)
			,
			...
			$this->settings->add_ItemToGroup( array(
				...
				'description' =>
					...
					sprintf(
						...
						$this->slplus->get_product_url('slp-enhanced-map')
					),
		...
		private function add_group_results_appearance( $section_name ) {
			...
			if (
				...
				$this->settings->add_ItemToGroup(array(
					'custom' =>
						sprintf(
							__('Add on packs like the %s add-on will add appearance control settings here.', 'store-locator-le'),
							$this->slplus->get_product_url('slp-experience')
						),
					'type' => 'details',
					'section' => $section_name,
					'group' => $group_name
			...
			$this->settings->add_ItemToGroup(array(
				...
				'description' =>
					...
					sprintf(
						...
						$this->slplus->get_product_url('slp-enhanced-search')
					),
		...
		function save_settings() {
			...
			if ( isset( $_POST['csl-slplus-theme'] ) ) {
		...
		function search_form_settings() {
			...
			$group_name	= __( 'Search Features', 'store-locator-le' );
			$this->settings->add_ItemToGroup(array(
				'section'    	=> $section_name,
				'group'      	=> $group_name,
				'label'		 	=>__( 'Radii Options', 'store-locator-le' ),
				'description'	=> __( 'Separate each number with a comma ",". Put parenthesis "( )" around the default.', 'store-locator-le' ),
				'setting'		=> 'radii',
				'use_prefix'	=> false,
				'value'			=> $this->slplus->options['radii']
			));

			// TODO: Convert this to a drop down type
			$html =
				"<div class='form_entry'>" .
			...
			$this->settings->add_ItemToGroup(
				array(
					...
					'custom'     => apply_filters( 'slp_settings_search_features', '' )
				)
			);
