...
if (! class_exists('SLP_BaseClass_Admin')) {
	...
	// TODO: is this needed here?  It is not used within this class.
    ...
    class SLP_BaseClass_Admin extends SLPlus_BaseClass_Object {
		 ...
		 * @return bool
		 */
		private function being_deactivated() {
        ...
        function do_admin_startup() {
			if ( $this->being_deactivated() ) { return; }
			...
			if ( $_REQUEST['action'] !== 'update' ) { return; }
			if (
	    ...
	    function save_my_settings() {
			...
			//
			if ( ! $this->slplus->needed_for_addon( get_class() . '::' . __FUNCTION__ ) && $this->settings_have_been_saved ) { return; }
