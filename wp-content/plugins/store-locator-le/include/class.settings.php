...
if ( !class_exists( 'SLP_Settings' ) ) {
	 ...
	 * @property-read   SLPlus_Settings_Section[] $sections
	 *
	 */
	class SLP_Settings extends SLPlus_BaseClass_Object {
		...
		function add_ItemToGroup($params) {
			...
			if (!isset($this->sections[$params['section']])) {
				return;
			}
		...
		function display() {
			...
			switch ($this->type) {
				...
				case 'subheader':
					...
					if (!empty($this->description)) {
						echo "<p class='slp_subheader_description' id='{$this->name}_p'>{$this->description}</p>";
					}
				...
				case 'icon':
					...
					echo "<img id='{$this->id}_icon' src='{$icon_src}' class='slp_settings_icon'>";
...
if ( !class_exists( 'wpCSL_settings__slplus' ) ) {
